...
void Shutdown()
{
	...
	//FlushCopyRightToState();
	//FlushUploadFileToState();
...
bool AppInitMain()
{
	...
	connOptions.m_master_nodes = gArgs.GetArgs("-masternode");
	connOptions.m_f_master_node = gArgs.GetBoolArg("-fmasternode", false);
	
	for(std::string& master : connOptions.m_master_nodes)  //add masternodes into VNodes
	{
		connOptions.m_added_nodes.push_back(master);
	}
    ...
    if (vpwallets.size() > 0) 
	{
		CWallet* pwalletMain = vpwallets[0];
