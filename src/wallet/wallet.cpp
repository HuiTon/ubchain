...
bool CWallet::CreateCopyRightTransaction(CTransactionRef& tx, CopyRight &right)
{
    CMutableTransaction txNew;
	txNew.fFundRecordTx = true;
	//txNew.vright.push_back(right);
	// Return the constructed transaction data.
	tx = MakeTransactionRef(std::move(txNew));
    ...
    LogPrintf("Fee Calculation: Fee: 0 \n");
	
    return true;
}

bool CWallet::CreateFundRecordTransaction(CTransactionRef& tx, FundRecord &fund)
{
    CMutableTransaction txNew;
	txNew.fFundRecordTx = true;
	txNew.vfund.push_back(fund);
	// Return the constructed transaction data.
	tx = MakeTransactionRef(std::move(txNew));
