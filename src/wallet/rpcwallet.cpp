...
static CTransactionRef SendMoneyAndSignCopyRight(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, const CCoinControl& coin_control, mapValue_t mapValue, std::string fromAccount, CopyRight &right)
{
	if (pwallet->GetBroadcastTransactions() && !g_connman) {
        throw JSONRPCError(RPC_CLIENT_P2P_DISABLED, "Error: Peer-to-peer functionality missing or disabled");
    }
    ...
    CTransactionRef tx;
    if (!pwallet->CreateCopyRightTransaction(tx, right)) {
        throw JSONRPCError(RPC_WALLET_ERROR, "CreateCopyRightTransaction failed");
    }
    CValidationState state;
    if (!pwallet->CommitTransaction(tx, std::move(mapValue), {} /* orderForm */, std::move(fromAccount), reservekey, g_connman.get(), state)) {
...
static CTransactionRef SendMoneyAndSignFundRecord(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, const CCoinControl& coin_control, mapValue_t mapValue, std::string fromAccount, FundRecord &fund)
{
	if (pwallet->GetBroadcastTransactions() && !g_connman) {
        throw JSONRPCError(RPC_CLIENT_P2P_DISABLED, "Error: Peer-to-peer functionality missing or disabled");
    }
    ...
    CTransactionRef tx;
    if (!pwallet->CreateFundRecordTransaction(tx, fund)) {
        throw JSONRPCError(RPC_WALLET_ERROR, "CreateCopyRightTransaction failed");
    }
    CValidationState state;
    if (!pwallet->CommitTransaction(tx, std::move(mapValue), {} /* orderForm */, std::move(fromAccount), reservekey, g_connman.get(), state)) {
...
UniValue signcopyright(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 4)
        throw std::runtime_error(
            "signcopyright \"name\" \"url\" \"address\" \"value\"\n"
            "\n sign a copy right.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
			"1. \"id\"            (string, required) the id of the digital copyright.\n"
            "2. \"name\"            (string, required) the name of the digital copyright.\n"
            "3. \"toaddress\"         (string, required) the ubchain address of the digital copyright owner.\n"
			"4. \"value\"           (string, required) the value of the digital copyright.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleRpc("signcopyright", "\"5a7d24cd665108c66b2d56146f244932edae4e2376b561b3d396d5ae017b9589\", \"sunshine.jpg\", \"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\", \"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\", 10000")
        );
    ...
    // Amount
    CAmount nAmount = Params().GetConsensus().nSignCopyRightFee;
    bool fSubtractFeeFromAmount = false;
	
    mapValue_t mapValue;
	CopyRight right;
	right.name = request.params[1].get_str();
	right.toaddress = request.params[2].get_str();
	right.value = atoi(request.params[3].get_str().c_str());
	right.status = 1;
	right.time = GetTime();
	right.id.SetHex(request.params[0].get_str());
    if(uploadFileMap.find(right.id) == uploadFileMap.end())
		throw JSONRPCError(RPC_TYPE_ERROR, "copyright file not found");
	
	UploadFileInfo& fileInfo = uploadFileMap[right.id];
	fileInfo.checks++;
	uploadFileMap[right.id].status = 1;    //checked
	
	std::vector<std::string>& masterIP = g_connman->GetMasterNodesIP();
	if(fileInfo.checks >= masterIP.size() * Params().GetConsensus().copyRightCheckRate)
	{
		if(copyRightTxMap.find(right.id) == copyRightTxMap.end())  //have not been signed
		{
			CCoinControl coin_control;
			EnsureWalletIsUnlocked(pwallet);

			CTransactionRef tx = SendMoneyAndSignCopyRight(pwallet, dest, nAmount, fSubtractFeeFromAmount, coin_control, std::move(mapValue), {} /* fromAccount */, right);
			uint256 hash = tx->GetHash(); 
			//LogPrintf("copyRightTxMap.insert copyright %s: tx:%s", right.id.ToString(), hash.ToString());
			//copyRightTxMap.insert(std::make_pair(right.id, hash));
			return hash.GetHex();
		}
		else throw JSONRPCError(RPC_TYPE_ERROR, "copyright have been signed");
	}
	else
	{
		std::vector<CInv> vInv;
		vInv.push_back(CInv(MSG_COPYRIGHT_CHECK, right.id));
		UniValue ret(UniValue::VOBJ);
		std::vector<std::string>& masterIP = g_connman->GetMasterNodesIP();
		for(std::string& ip : masterIP)
		{
			CNode* pnode = g_connman->FindNodeFromIP(ip);
			if(pnode == nullptr) continue;
			LogPrint(BCLog::NET, "PushMessage:MSG_COPYRIGHT_CHECK (%s) will be broadcasted to peer=%d\n",right.id.GetHex(), pnode->addr.ToStringIP());
			const CNetMsgMaker msgMaker(pnode->GetSendVersion());
			g_connman->PushMessage(pnode, msgMaker.Make(NetMsgType::INV, vInv));
		}
		return right.id.GetHex() + "is checking";
	}
	
}

UniValue unsigncopyright(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "unsigncopyright \"id\"\n"
            "\n don't sign a copy right.\n"
            "\nArguments:\n"
			"1. \"id\"            (string, required) the id of the digital copyright.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleRpc("unsigncopyright", "\"5a7d24cd665108c66b2d56146f244932edae4e2376b561b3d396d5ae017b9589\"")
        );
...
UniValue signcopyrighttransaction(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            "signcopyrighttransaction \"id\" \"name\" \"fromaddress\" \"toaddress\" \"value\" \"prevtx\" \n"
            "\nsign a copyright transaction.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
			"1. \"id\"            (string, required) the id of the digital copyright.\n"
            "2. \"name\"            (string, required) the name of the digital copyright.\n"
			"3. \"fromaddress\"         (string, required) the ubchain address of the digital copyright seller.\n"
            "4. \"toaddress\"         (string, required) the ubchain address of the digital copyright buyer.\n"
			"5. \"value\"           (string, required) the value of the digital copyright.\n"
			"6. \"hash\"           (string, required) the hash of the previous tx.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleRpc("signcopyrighttransaction", "\"5a7d24cd665108c66b2d56146f244932edae4e2376b561b3d396d5ae017b9589\", \"sunshine.jpg\", \"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\", 10000, \"6b7d24cd665108c66b2d56146f244932edae4e2376b561b3d396d5ae017b9589\"")
        );
	...
	CopyRight right;
	right.name = request.params[1].get_str();
	right.prevtx.SetHex(request.params[5].get_str());
	right.fromaddress = request.params[2].get_str();
	right.toaddress = request.params[3].get_str();
	right.value = atoi(request.params[4].get_str().c_str());
	right.status = 2;
	right.time = GetTime();
	right.id.SetHex(request.params[0].get_str());
    
	if(copyRightTxMap.find(right.id) == copyRightTxMap.end())  //have not been signed
		 throw JSONRPCError(RPC_TYPE_ERROR, "copyright have not been signed");
	if(copyRightTxMap[right.id] != right.prevtx)  //have not been signed
		 throw JSONRPCError(RPC_TYPE_ERROR, "copyright prev tx is not correct");
    ...
    CTransactionRef tx = SendMoneyAndSignCopyRight(pwallet, dest, nAmount, fSubtractFeeFromAmount, coin_control, std::move(mapValue), {} /* fromAccount */, right);
    uint256 hash = tx->GetHash(); 
	//LogPrintf("copyRightTxMap.update copyright %s: tx:%s", right.id.ToString(), hash.ToString());
	//copyRightTxMap[right.id] = hash;
	
	return hash.GetHex();
}

UniValue signdepositrecord(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signdepositrecord \"address\" \"value\"\n"
            "\n sign a deposit record(.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
			"1. \"address\"            (string, required) the address of the depositer.\n"
			"2. \"value\"           (string, required) the value of the deposit record.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleRpc("signdepositrecord", "\"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\", 10000")
        );
	...
	CTransactionRef tx = SendMoneyAndSignFundRecord(pwallet, dest, nAmount, fSubtractFeeFromAmount, coin_control, std::move(mapValue), {} /* fromAccount */, fund);
	uint256 hash = tx->GetHash(); 
	
	return hash.GetHex();	
}

UniValue signwithdrawrecord(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signwithdrawrecord \"address\" \"value\"\n"
            "\n sign a withdraw record(.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
			"1. \"address\"            (string, required) the address of the withdrawer.\n"
			"2. \"value\"           (string, required) the value of the withdraw record.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleRpc("signwithdrawrecord", "\"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\", 10000")
        );
...
UniValue getblockfundrecord(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            "getblockfundrecord \"blockhash\"\n"
            "\nReturn the fund record of the block..\n"
            "\nArguments:\n"
            "1. \"hash\"            (string, required) The hash of the tx\n"
            "\nExamples:\n"
            + HelpExampleRpc("getblockfundrecord",  "\"5a7d24cd665108c66b2d56146f244932edae4e2376b561b3d396d5ae017b9589\"")
        );
...
UniValue gettxfundrecord(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            "gettxfundrecord \"id\"\n"
            "\nReturn the fund record of the tx..\n"
            "\nArguments:\n"
            "1. \"hash\"            (string, required) The hash of the tx\n"
            "\nExamples:\n"
            + HelpExampleRpc("gettxfundrecord",  "\"5a7d24cd665108c66b2d56146f244932edae4e2376b561b3d396d5ae017b9589\"")
        );
	...
	if(tx->vfund.size() == 0)
	{
		throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No such copyright data found for this copyright.");
	}
...
{ //  category              name                                actor (function)                argNames
	...
	{ "wallet",             "signcopyright",                    &signcopyright,                 {"id", "name","address","value"} },
	{ "wallet",             "signcopyrighttransaction",         &signcopyrighttransaction,      {"id","name","fromaddress","toaddress","value","txid"} },
	{ "wallet",             "signdepositrecord",                &signdepositrecord,             {"address","value"} },
	{ "wallet",             "signwithdrawrecord",               &signwithdrawrecord,             {"address","value"} },
	{ "wallet",             "gettxfundrecord",                  &gettxfundrecord,                {"hash"} },
	{ "wallet",             "getblockfundrecord",               &getblockfundrecord,             {"blockhash"} },
